...
// Shared logic for the "World Cup & Soccer" interest. World Cup viewing is location-first,
// not date-first: fans already know when interesting matches kick off, so we present every
// PLACE with viewing (aggregated across all its sessions into one entry with a date span)
// rather than a day-by-day list. Used by both the standalone browse endpoint
// (/api/worldcup) and the weekly planner (/api/plan), so the two always agree.
...
// Fetch and aggregate all World Cup viewing into per-spot entries, with best-effort travel
// estimates from the user's home/office. Travel is informational only — spots are never
// filtered out by budget, working hours, or distance.
export async function getWorldCupSpots(profile: {
  homeAddress?: string
  officeAddress?: string
}): Promise<WorldCupSpotsResult> {
  ...
  for (const row of rows) {
    ...
    if (!existing) {
      groups.set(key, { row, firstDate: startDate, lastDate: endDate, isMultiDay: rowMultiDay, sessions: 1, coord })
    } else {
      existing.sessions += 1
